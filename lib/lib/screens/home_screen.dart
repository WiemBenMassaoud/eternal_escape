...
class _HomeScreenState extends State<HomeScreen> {
  ...
  // ✅ INITIALISATION DES LOGEMENTS AVEC IDs UNIQUES
  void _initializeLogements() {
    var box = Hive.box<Logement>('logements');

    if (box.isEmpty) {
      print('📦 Initialisation des logements avec IDs uniques...');
      _addRealEstateData(box);
    } else {
      print('ℹ️ ${box.length} logements déjà présents');
      
      // ✅ Vérifier et corriger les IDs manquants
      ...
      if (needsUpdate) {
        print('✅ IDs des logements mis à jour');
      }
    }
  }
  ...
  // ✅ AJOUT DES LOGEMENTS AVEC IDs UNIQUES
  void _addRealEstateData(Box<Logement> box) {
    ...
    for (var logement in logements) {
      box.add(logement);
      print('✅ Logement ajouté: ${logement.nom} (ID: ${logement.id})');
    }
  }
